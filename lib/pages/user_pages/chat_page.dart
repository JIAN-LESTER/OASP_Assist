  ...
  class _ChatPageState extends State<ChatPage> with TickerProviderStateMixin {
    ...
    try {
      ...
      // ✅ Only check escalations once on initial load
      // Real-time updates are handled by the listener in ChatProvider
      // if (!hasMessages) {
  ...
  //   try {
  //     print('🔍 Initial check for escalation responses: ${widget.conversationId}');
  ...
  //         print('ℹ️ Staff response already exists');
  //         continue;
  //       }

  //       // Check in-memory
  //       final existingInMemory = chatProvider.messages.any(
  //         (msg) => msg.content == staffMessageContent && msg.sender == 'staff',
  //       );

  //       if (existingInMemory) {
  //         print('ℹ️ Staff response already in memory');
  //         continue;
  //       }

  //       print('📝 Adding staff response');
                                    ...
                                    : // Bot/Staff messages - markdown support
                                      MarkdownBody(
                                        ...
                                        styleSheet: MarkdownStyleSheet(
                                          p: TextStyle(
                                            color: message.sender == 'staff' || message.sender == 'admin'
...
Future<void> _processManualEscalation(Message message) async {
  ...
  try {
    final bool? shouldEscalate = await showDialog<bool>(
      context: context,
      barrierDismissible: true,
      builder: (BuildContext dialogContext) { // ✅ Use separate context variable
