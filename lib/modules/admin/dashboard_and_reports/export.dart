...
class ExportService {
        ...
        build:
            (context) => [
              ...
              _buildStatsTable([
                ...
                [
                  'Escalation Rate',
                  '${inq?.escalationRate.toStringAsFixed(1) ?? 0}%',
                ],
                [
                  'Resolution Rate',
                  '${inq?.resolutionRate.toStringAsFixed(1) ?? 0}%',
                ],
              ]),
              pw.SizedBox(height: 20),
              ...
              if (inq?.inquiryTrend.isNotEmpty ?? false) ...[
                ...
                _buildKeyValueTable(
                  Map.fromEntries(
                    inq!.inquiryTrend.map((e) => MapEntry(e.date, e.count)),
                  ),
                ),
                pw.SizedBox(height: 20),
              ],
              ...
              if (inq?.escalationsOverTime.isNotEmpty ?? false) ...[
                ...
                _buildKeyValueTable(
                  Map.fromEntries(
                    inq!.escalationsOverTime.map(
                      (e) => MapEntry(e.date, e.count),
                    ),
                  ),
                ),
                pw.SizedBox(height: 20),
        ...
        build:
            (context) => [
              ...
              _buildStatsTable([
                ...
                [
                  'Escalation Rate',
                  '${data?.escalationRate.toStringAsFixed(1) ?? 0}%',
                ],
                [
                  'Resolution Rate',
                  '${data?.resolutionRate.toStringAsFixed(1) ?? 0}%',
                ],
              ]),
              pw.SizedBox(height: 20),
              ...
              if (data?.inquiryTrend.isNotEmpty ?? false) ...[
                ...
                _buildKeyValueTable(
                  Map.fromEntries(
                    data!.inquiryTrend.map((e) => MapEntry(e.date, e.count)),
                  ),
                ),
                pw.SizedBox(height: 20),
              ],
              ...
              if (data?.staffPerformance.isNotEmpty ?? false) ...[
                ...
                _buildKeyValueTable(
                  data!.staffPerformance.map(
                    (k, v) => MapEntry(k, v.toStringAsFixed(1) + '%'),
                  ),
                ),
                pw.SizedBox(height: 20),
              ],
            ...
            (context) => [
              ...
              _buildStatsTable([
                [
                  ...
                  formatTime(data?.averageResponseTime ?? 0),
                ],
                ['Total Conversations', '${data?.totalConversations ?? 0}'],
                [
                  'Avg Messages/User',
                  '${data?.averageMessagesPerUser.toStringAsFixed(1) ?? 0}',
                ],
                [
                  'Chat Limit Reach Rate',
                  '${data?.chatLimitReachRate.toStringAsFixed(1) ?? 0}%',
                ],
                [
                  'Escalation Limit Rate',
                  '${data?.escalationLimitReachRate.toStringAsFixed(1) ?? 0}%',
                ],
              ]),
              pw.SizedBox(height: 20),
              ...
              if (data?.conversationsOverTime.isNotEmpty ?? false) ...[
                ...
                _buildKeyValueTable(
                  Map.fromEntries(
                    data!.conversationsOverTime.map(
                      (e) => MapEntry(e.date, e.count),
                    ),
                  ),
                ),
              ...
              if (data?.escalationLimitReachTrend.isNotEmpty ?? false) ...[
                ...
                _buildKeyValueTable(
                  Map.fromEntries(
                    data!.escalationLimitReachTrend.map(
                      (e) => MapEntry(e.date, e.count),
                    ),
                  ),
                ),
        ...
        build:
            (context) => [
              ...
              _buildStatsTable([
                ...
                [
                  'Without Scholarship',
                  '${data?.usersWithoutScholarship ?? 0}',
                ],
              ]),
              pw.SizedBox(height: 20),
  ...
  static Future<String> exportDashboardToCSV({
    ...
    if (inq?.recentLogs.isNotEmpty ?? false) {
      rows.addAll(
        inq!.recentLogs.take(10).map((log) => [log.time, log.action]),
      );
    }
    ...
    if (inq?.msgLogs.isNotEmpty ?? false) {
      rows.addAll(
        inq!.msgLogs.take(10).map((log) => [log.time, log.user, log.message]),
      );
    }
    ...
    if (ad?.topEscalatedMessages.isNotEmpty ?? false) {
      rows.addAll(
        ad!.topEscalatedMessages
            .take(10)
            .map((msg) => [msg.userMessage, msg.status]),
      );
    }
  ...
  static Future<String> exportInquiryTrendsToCSV({
    ...
    if (data?.categoryDistribution.isNotEmpty ?? false) {
      rows.addAll(
        data!.categoryDistribution.entries.map((e) => [e.key, e.value]),
      );
    }
    ...
    if (data?.staffPerformance.isNotEmpty ?? false) {
      rows.addAll(
        data!.staffPerformance.entries.map(
          (e) => [e.key, e.value.toStringAsFixed(1)],
        ),
      );
    }
  ...
  static Future<String> exportChatbotUsageToCSV({
    ...
    List<List<dynamic>> rows = [
      ...
      [
        'Average Response Time',
        '${data?.averageResponseTime.toStringAsFixed(2) ?? 0}s',
      ],
      ['Total Conversations', data?.totalConversations ?? 0],
      [
        'Avg Messages/User',
    ...
    if (data?.peakUsageByHour.isNotEmpty ?? false) {
      rows.addAll(
        data!.peakUsageByHour.entries.map((e) => ['${e.key}:00', e.value]),
      );
    }
    ...
    if (data?.escalationLimitReachTrend.isNotEmpty ?? false) {
      rows.addAll(
        data!.escalationLimitReachTrend.map((d) => [d.date, d.count]),
      );
    }
  ...
  static Future<String> exportUserDemographicsToCSV({
    ...
    if (data?.scholarshipDistribution.isNotEmpty ?? false) {
      rows.addAll(
        data!.scholarshipDistribution.entries.map((e) => [e.key, e.value]),
      );
    }
