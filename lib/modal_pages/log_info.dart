...
void showLogsInfoModal(
  ...
  // Helper: find bot reply corresponding to a user message
  String? findBotReplyForUser(
    Map<String, dynamic> userMessage,
    List<Map<String, dynamic>> messages,
  ) {
    final userTime = userMessage['responded_at'];

    final botMessage = messages.firstWhere(
      (msg) => msg['sender'] == 'bot' && msg['sent_at'] == userTime,
      orElse: () => {},
    );
  ...
  // Find the current user message inside the full messages list
  final userMessage = messages.firstWhere(
    (msg) =>
        msg['sender'] == 'user' && msg['responded_at'] == data['responded_at'],
    orElse: () => {},
  );
  ...
  final botResponse =
      userMessage.isNotEmpty
          ? findBotReplyForUser(userMessage, messages)
          : null;
