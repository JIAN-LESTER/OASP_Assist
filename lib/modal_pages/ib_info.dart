...
Future<void> _handleDeleteDocument(
  ...
  try {
    ...
    final weaviateId = docData['weaviate_id'];
    ...
    // Delete scholarships first (most likely to fail)
    ...
    // Delete main information_bank doc
    batch.delete(firestore.collection('information_bank').doc(doc.id));

    // Delete related admission doc
    batch.delete(firestore.collection('admissions').doc(doc.id));

    // Commit batch for main docs
    await batch.commit();
    print("✅ Deleted main documents");

    // Pop loading dialog and confirmation dialog
    if (context.mounted) {
  ...
  try {
    ...
    for (final chunkId in chunkIds) {
      try {
        ...
        print("⚠️ Failed to delete Weaviate chunk $chunkId: $e");
      }
    }
    ...
    if (weaviateId != null && weaviateId.isNotEmpty) {
      try {
        ...
        if (response.statusCode >= 400) {
          ...
          print("✅ Weaviate parent document deleted");
        }
      } catch (e) {
        print("⚠️ Error deleting parent Weaviate doc: $e");
      }
    }
  } catch (e) {
